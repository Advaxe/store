...
import PlusDetail from "./PlusDetail";
const ListPageArticle = () => {
  ...
  const deleteItems = async (itemsIds) => {
    try {
      ...
      const res = await fetchApi("/article/delete/", {
        method: 'POST',
        body: form
      })
      dispacth(setToastAction({ severity: 'success', summary: ' supprimé', detail: "L'article a été supprimé avec succès", life: 3000 }))
      fetchArticle()
      setSelectAll(false)
    ...
    confirmDialog({
      ...
      message: <div className="d-flex flex-column align-items-center">
        {inViewMenuItem ? <>
          ...
          <div className="font-bold text-center my-2">{inViewMenuItem?.NAME_ARTICLE} </div>
  ...
  const fetchArticle = useCallback(async (currentLazy) => {
    try {
      setLoading(true)
      const baseurl = `/article/all?`;
      var url = baseurl;
      ...
      setArticles(response.result.data)
...
const [selectedArticles, setSelectedArticles] = useState(null)
  ...
  const handleOpenDialog = () => {
    setSelectedArticles(inViewMenuItem)
    ...
    <>
      ...
      <div className="px-4 py-3 main_content">
        <div className="d-flex align-items-center justify-content-between">
          ...
          <Button label="Nouvel article" icon="pi pi-plus" size="small" onClick={() => {
          ...
          <div className="shadow rounded mt-3 pr-1 bg-white">
            ...
            >
              ...
              <Column
                ...
                frozen
                body={(item) => {
                  const css = `.p-image-preview-indicator {border-radius: 50%}`;
                  return (
                    <>
                      <Image
              ...
              <Column field="PRICE_ARTICLE" frozen header="Prix" sortable body={item => {
                return (
                  <span>{item.PRICE_ARTICLE}</span>
                )
              }} />
              <Column field="DESIGNATION" frozen header="Categorie" sortable body={item => {
                return (
              ...
              <Column field="" header="Actions" alignFrozen="right" frozen body={item => {
                const items = [
                  ...
                  },
                  {
                    template: (deleteItem, options) => {
                      return (
                        <Link to={`/article/edit/${inViewMenuItem?.ARTICLE_ID}`} className="p-menuitem-link">
                  ...
                  },
                  {
                    template: (deleteItem, options) => {
                      return (
                        <a href="#" className="p-menuitem-link text-danger" onClick={e => handleDeletePress(e, [inViewMenuItem.ARTICLE_ID])}>
...
export default ListPageArticle